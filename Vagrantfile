...
#  config.vm.network :bridged
  ...
  config.vm.provision :shell, :inline => "sudo apt-get update && sudo apt-get install samba --assume-yes && sudo mv /etc/samba/smb.conf /etc/samba/smb.conf.default"
  # Install full nublic
  config.vm.provision :shell, :inline => "sudo apt-get install --assume-yes --force-yes nublic-notification libtomcat7-java nublic-file-watcher nublic-app-browser nublic"
