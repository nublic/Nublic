 ...
 * 
 * Taken from
 * http://thezukunft.com/2010/02/05/an-editable-label-for-gwt-with-uibinder
 * -and-eventhandlers/
 * 
 * @author Jonas Huckestein
 * 
 */
public class EditableLabel extends Composite implements HasValue<String>, HasText {
		...
		editBox.addKeyPressHandler(new KeyPressHandler() {
			...
			public void onKeyPress(KeyPressEvent event) {
				...
				if (event.getNativeEvent().getKeyCode() == KeyCodes.KEY_ENTER) {
					...
					editBox.setText(editLabel.getText()); // reset
				}
			}
		});
	}
