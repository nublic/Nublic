...
def get_all_mirrors(use_dbus=False):
    notSplitted = _call_mirror_method_return(
        lambda i: i.get_all_mirrors(), use_dbus)
    if notSplitted == '':
        return []
    # Take elements apart
...
def create_mirror(name, owner, use_dbus=False):
    mid = _call_mirror_method_return(
        lambda i: i.create_mirror(name, owner), use_dbus)
    return Mirror(mid)
...
class Mirror:
    ...
    def get_path(self):
        return MIRROR_DATA_ROOT + unicode(self._id)

    def delete(self, remove_in_file_system):
        _call_mirror_method(lambda i: i.delete_mirror(
